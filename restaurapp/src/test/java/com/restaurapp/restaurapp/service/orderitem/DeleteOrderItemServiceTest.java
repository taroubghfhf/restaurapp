...
class DeleteOrderItemServiceTest {
    ...
    @Test
    void execute_WhenOrderItemExists_ShouldDeleteOrderItem() {
        // Given
        doReturn(Optional.of(orderItem)).when(orderItemRepositoryJpa).findById(anyLong());
        doNothing().when(orderItemRepositoryJpa).delete(any(OrderItem.class));
        ...
        verify(orderItemRepositoryJpa, times(1)).delete(orderItem);
    }
    ...
    void execute_WhenOrderItemDoesNotExist_ShouldThrowException() {
        ...
        assertEquals("El detalle de comanda no existe", exception.getMessage());
        verify(orderItemRepositoryJpa, times(1)).findById(1L);
        verify(orderItemRepositoryJpa, never()).delete(any(OrderItem.class));
    }
}
